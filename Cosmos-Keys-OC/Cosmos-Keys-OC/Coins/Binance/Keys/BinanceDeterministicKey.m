...
#import "BinanceDeterministicKey.h"
#import "SHAHash.h"
#include "memzero.h"
#import "secp256k1_zkp.h"
#import "Categories.h"
#import "BinanceECKey.h"
#import "secp256k1.h"

@implementation BinanceDeterministicKey

- (instancetype)initWithSeed:(NSData*)seed {
    // generate private key
    NSData *masterPriKey = [BinanceDeterministicKey createMasterPrivateKey:seed forKey:[@"Bitcoin seed" dataUsingEncoding:NSUTF8StringEncoding]];
...
+ (NSData *)createMasterPrivateKey:(NSData*)seed forKey:(NSData *)key{
    // selection algorithm
    NSData *masterPriKey = [SHAHash Hmac512:key Data:seed];
    return masterPriKey;
}
- (DeterministicKey*)privChild:(ChildNumber*)childNumber {
    ...
    MBigNumber *N = [[MBigNumber alloc] initWithBigNum:curve->order];
    MBigNumber *ki;
    while (true) {
        bool failed = false;
    ...
    // [3] generate BinanceDeterministicKey object
    DeterministicKey *rawKey = [[BinanceDeterministicKey alloc] initWithPri:ki.toDataBE Pub:nil Code:[i subdataWithRange:NSMakeRange(32, 32)]];
    return rawKey;
}
...
- (DeterministicKey*)pubChild:(ChildNumber *)childNumber {
    ...
    NSData *i = [SHAHash Hmac512:chainCode Data:data];
    NSData *il = [i subdataWithRange:NSMakeRange(0, 32)];
    MBigNumber *ilInt = [[MBigNumber alloc] initWithData:il];
    
    
    // [2] private key to public point
    MBigNumber *N = [[MBigNumber alloc] initWithBigNum:curve->order];
    MBigNumber *pri = ilInt;
    ...
    // [5] generate BinanceDeterministicKey object
    DeterministicKey *rawKey = [[BinanceDeterministicKey alloc] initWithPri:nil Pub:[NSData dataWithBytes:pub_key length:33] Code:[i subdataWithRange:NSMakeRange(32, 32)]];
    return rawKey;
}

- (id)toECKey {
    return [[BinanceECKey alloc] initWithKey:privateKey Pub:publicKey];
}
